...
from medical_records.services.orthanc_service import (
    find_studies_by_encounter,
    get_study_series,
    get_instance_preview,
)
...
@require_GET
def dicom_encounters_api(request):
...
@require_GET
def dicom_series_api(request, study_id):
    if not request.user.is_authenticated:
        return JsonResponse({
            "success": False,
            "error": "Authentication required",
...
@require_GET
def dicom_preview_api(request, instance_id):
    if not request.user.is_authenticated:
        return JsonResponse({
            "success": False,
            "error": "Authentication required",
    ...
    try:
        preview = get_instance_preview(instance_id)
